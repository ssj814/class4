...
<link rel="stylesheet" type="text/css" href="resources/css/trainer/traineradd.css"> 
...
<div class="trainerContainer">
    <form action="trainer_modify" method="post" enctype="multipart/form-data">
        ...
        <div class="bodyContainer">
            ...
            <input type="text" id="nickname" name="nickname" value="${info.nickname}" required>
        </div>
        ...
        <div class="bodyContainer">
            ...
            <input type="text" id="center" name="center_name" value="${info.center_name}" required>
        </div>

        <div class="bodyContainer">
            <label class="compulsory">소속 센터 주소</label>
            <div>
                <input type="text" id="postcode" name="center_postcode" value="${info.center_postcode}" required>
                <input type="button" onclick="findpostcode()" value="우편번호 찾기">
                <input type="text" id="centerAddress" name="center_address1" value="${info.center_address1}" required>
                <input type="text" id="centerAddressDetail" name="center_address2" value="${info.center_address2}" required>
            </div>
        </div>

        <div class="bodyContainer">
            <label class="compulsory">한줄 소개글 (대표 소개글로 설정됩니다.)</label>
            <input type="text" id="intro" name="intro" value="${info.intro}" required>
        </div>

        <div class="bodyContainer">
            <label class="compulsory">소개글 및 소개사진</label>
            <textarea id="introDetail" name="content" cols="30" rows="8" required>${info.content}</textarea>
        ...
        <div class="bodyContainer">
            ...
            <div>
                <select name="lesson_area1" id="locSelect" required></select>
                <select name="lesson_area2" id="locSelect2" required></select>
            </div>
        </div>

        <div class="bodyContainer">
            <label class="compulsory">레슨 프로그램</label>
            <c:forEach items="${lsProgramOptions}" var="lsProgram" varStatus="status">
                <input type="checkbox" class="lesson_program me-1" name="lesson_program" value="${lsProgram}" id="program${status.index}">
                <label for="program${status.index}" class="me-3">${lsProgram}</label>
            </c:forEach>
        </div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		// 수정전 지도종목 데이터 가져오기
		var field = `${info.field}`.split(',');
		$(".field").each(function(idx,data){
		...
		//수정전 프로그램 데이터 가져오기
		var lessonProgram = `${info.lesson_program}`.split(',');
		$(".lesson_program").each(function(idx,data){
			if(lessonProgram.includes(data.value)){
				$(data).attr('checked', 'checked');	
			}
		});
